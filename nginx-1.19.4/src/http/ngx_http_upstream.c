...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (rc == NGX_DECLINED) {
        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_BACKEND_DOWN);
        return;
    }
    ...
    ngx_uint_t ft_type)
{
    ...
    if (u->peer.cached && (ft_type == NGX_HTTP_UPSTREAM_FT_ERROR || ft_type == NGX_HTTP_UPSTREAM_FT_BACKEND_DOWN)) {
    ...
    default:
        status = (ft_type == NGX_HTTP_UPSTREAM_FT_BACKEND_DOWN || ft_type == NGX_HTTP_UPSTREAM_FT_NOLIVE) ? NGX_HTTP_SERVICE_UNAVAILABLE : NGX_HTTP_BAD_GATEWAY;
    }
    ...
    *u->cleanup = NULL;
    ...
    if (r->cache) {
        ...
        if (u->cacheable) {
            ...
            if (rc == NGX_HTTP_BAD_GATEWAY || rc == NGX_HTTP_SERVICE_UNAVAILABLE || rc == NGX_HTTP_GATEWAY_TIME_OUT) {
                time_t  valid;
